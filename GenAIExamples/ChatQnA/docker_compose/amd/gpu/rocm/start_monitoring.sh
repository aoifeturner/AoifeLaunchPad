...
# Start ChatQnA Monitoring Stack
# This script starts Prometheus and Grafana for monitoring ChatQnA performance

set -e

echo "🚀 Starting ChatQnA Monitoring Stack"
echo "===================================="
...
# Check if ChatQnA services are running
check_chatqna() {
    ...
    if docker ps --format "{{.Names}}" | grep -q "chatqna-backend-server"; then
        ...
        print_error "ChatQnA services are not running. Please start them first:"
...
start_monitoring() {
    ...
    if docker ps --format "{{.Names}}" | grep -q "chatqna-prometheus"; then
    ...
    if docker ps --format "{{.Names}}" | grep -q "chatqna-grafana"; then
...
show_access_info() {
    ...
    echo "  - ChatQnA MegaService Dashboard"
    ...
    echo "  4. Run your benchmarks while monitoring"
    echo ""
