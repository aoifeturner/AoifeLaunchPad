...
# ChatQnA vLLM Management Script
# This script provides a unified interface for all ChatQnA operations with vLLM
...
setup_environment() {
    ...
    print_status "Setting up environment variables for vLLM..."
...
setup_lightweight() {
    print_header "Setting up Lightweight ChatQnA vLLM Environment"
    check_docker
    check_requirements
    
    print_status "Setting up lightweight environment variables for vLLM..."
    source set_env_lightweight.sh
    
    print_status "Lightweight vLLM environment setup complete!"
}

# Function to start ChatQnA vLLM services
start_services() {
...
# Function to stop ChatQnA vLLM services
stop_services() {
...
run_quick_eval() {
    print_header "Running Quick Evaluation with vLLM"
    check_docker
    
    print_status "Running quick evaluation with vLLM..."
    ./quick_test_chatqna.sh
    
    print_status "Quick evaluation with vLLM complete!"
}

# Function to run full evaluation
run_full_eval() {
    print_header "Running Full Evaluation with vLLM"
    check_docker
    
    print_status "Setting up evaluation environment for vLLM..."
    ./quick_eval_setup.sh
    
    print_status "Full evaluation setup with vLLM complete!"
...
# Function to check vLLM service status
check_status() {
    print_header "vLLM Service Status"
    check_docker
    
    print_status "ChatQnA vLLM Services:"
...
cleanup() {
    print_header "Cleaning Up vLLM Services"
    check_docker
    
    print_warning "This will stop all vLLM services and remove containers. Continue? (y/N)"
    read -r response
    if [[ "$response" =~ ^[Yy]$ ]]; then
        print_status "Stopping all vLLM services..."
        docker compose -f compose_vllm.yaml down
        docker compose -f compose.telemetry.yaml down
        ...
        print_status "vLLM cleanup complete!"
    else
...
show_help() {
    print_header "ChatQnA vLLM Management Script Help"
    echo "Usage: $0 [OPTION]"
    echo ""
    echo "Options:"
    echo "  1) setup          - Setup vLLM environment variables"
    echo "  2) setup-light    - Setup lightweight vLLM environment"
    echo "  3) start          - Start ChatQnA vLLM services"
    echo "  4) stop           - Stop ChatQnA vLLM services"
    echo "  5) restart        - Restart ChatQnA vLLM services"
    echo "  6) monitor-start  - Start monitoring stack"
    echo "  7) monitor-stop   - Stop monitoring stack"
    echo "  8) quick-eval     - Run quick evaluation with vLLM"
    echo "  9) full-eval      - Setup full evaluation with vLLM"
    echo "  10) logs          - Show vLLM service logs"
    echo "  11) monitor-logs  - Show monitoring logs"
    echo "  12) status        - Check vLLM service status"
    echo "  13) cleanup       - Clean up all vLLM services"
    echo "  14) menu          - Show interactive menu"
    echo "  15) help          - Show this help"
    echo ""
    echo "Examples:"
    echo "  $0 setup          # Setup vLLM environment"
    echo "  $0 start          # Start vLLM services"
...
show_menu() {
    while true; do
        print_header "ChatQnA vLLM Management Menu"
        echo "1.  Setup vLLM Environment"
        echo "2.  Setup Lightweight vLLM Environment"
        echo "3.  Start vLLM Services"
        echo "4.  Stop vLLM Services"
        echo "5.  Restart vLLM Services"
        echo "6.  Start Monitoring"
        echo "7.  Stop Monitoring"
        echo "8.  Run Quick Evaluation (vLLM)"
        echo "9.  Setup Full Evaluation (vLLM)"
        echo "10. Show vLLM Service Logs"
        echo "11. Show Monitoring Logs"
        echo "12. Check vLLM Status"
        echo "13. Cleanup vLLM Services"
        echo "14. Help"
        echo "15. Exit"
        echo ""
